...
export class LuaDebug extends DebugSession {
  ...
  protected setupProcessHanlders() {
    ...
    this.netMgr.on("C2S_LuaPrint", (result) => {
      ...
      if (this.printDate) {
        content =
          this.FormatDate(new Date(), "[yyyy-MM-dd hh:mm:ss] ") + content;
      }
  ...
  protected launchRequest(
    ...
    this.luaStartProc.on("error", (error) => {
      this.sendEvent(
        new OutputEvent(
          "LuaDebug->launchRequest.proc.error: " + error.message + "\n"
        )
      );
    });
  ...
  protected setBreakPointsRequest(
    ...
    if (
      this.netMgr != null &&
      this.netMgr.getSocketState() == ClientStatus.connected
    ) {
      let data = this.bpMgr.getClientBreakPointInfo(path);
      this.netMgr.sendMsg(
        LuaDebuggerEvent.S2C_SetBreakPoints,
        data ? [data] : null,
        this.isHitBreak == true
          ? this.netMgr.mainSocket
          : this.netMgr.breakPointSocket
      );
    }
    this.sendResponse(response);
  }
  ...
  protected variablesRequest(
    ...
    let that: LuaDebug = this;
    let luaDebugVarInfo: LuaDebugVarInfo = this.scopeMgr.getDebugVarsInfoByVariablesReference(
      args.variablesReference
    );
    if (luaDebugVarInfo) {
      this.scopeMgr.getVarsInfos(args.variablesReference, function (variables) {
        let newVariables = [];
        if (variables) {
              ...
              if (that.exConfig && that.exConfig["luaide-lite.numberRadix"]) {
                ...
                if (radixes && radixes.length > 0) {
                  ...
                  for (let i = 0; i < radixes.length; i++) {
                    ...
                    if (radix == 16) {
                      newValue +=
                        "0x" + parseInt(newVariable.value).toString(radix);
                    } else {
        ...
        if (luaDebugVarInfo.name != "_userdata")
  ...
  protected setVariableRequest(
    ...
    let varInfo: LuaDebugVarInfo = this.scopeMgr.getDebugVarsInfoByVariablesReference(
      args.variablesReference
    );
    if (varInfo) {
      let that = this;
  ...
  public convertToServerPath(path: string): string {
    ...
    for (let index = 0; index < paths.length; index++) {
      ...
      while (true) {
        if (
          clientPaths[clientPathsCount--] != serverPaths[serverPathsCount--]
        ) {
          isHit = false;
          break;
        } else {
  ...
  private initPathMaps(scripts: Array<string>) {
    ...
    for (let index = 1; index < paths.length; index++) {
      ...
      for (let k = 0; k < tempPaths.length; k++) {
        if (
          addPath == tempPaths[k] ||
          addPath.indexOf(tempPaths[k]) > -1 ||
          tempPaths[k].indexOf(addPath) > -1
        ) {
          isAdd = false;
          break;
        }
